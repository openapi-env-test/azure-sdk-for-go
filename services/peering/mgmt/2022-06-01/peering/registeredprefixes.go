...
// Validate validates an existing registered prefix with the specified name under the given subscription, resource
// group and peering.
// Parameters:
// resourceGroupName - the name of the resource group.
// peeringName - the name of the peering.
// registeredPrefixName - the name of the registered prefix.
func (client RegisteredPrefixesClient) Validate(ctx context.Context, resourceGroupName string, peeringName string, registeredPrefixName string) (result RegisteredPrefix, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/RegisteredPrefixesClient.Validate")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.ValidatePreparer(ctx, resourceGroupName, peeringName, registeredPrefixName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "peering.RegisteredPrefixesClient", "Validate", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "peering.RegisteredPrefixesClient", "Validate", resp, "Failure sending request")
		return
	}

	result, err = client.ValidateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "peering.RegisteredPrefixesClient", "Validate", resp, "Failure responding to request")
		return
	}
...
func (client RegisteredPrefixesClient) ValidatePreparer(ctx context.Context, resourceGroupName string, peeringName string, registeredPrefixName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"peeringName":          autorest.Encode("path", peeringName),
		"registeredPrefixName": autorest.Encode("path", registeredPrefixName),
		"resourceGroupName":    autorest.Encode("path", resourceGroupName),
...
// http.Response Body if it receives an error.
func (client RegisteredPrefixesClient) ValidateSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// ValidateResponder handles the response to the Validate request. The method always
// closes the http.Response Body.
func (client RegisteredPrefixesClient) ValidateResponder(resp *http.Response) (result RegisteredPrefix, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
