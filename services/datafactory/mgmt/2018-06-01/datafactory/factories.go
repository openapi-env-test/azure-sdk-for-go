...
// GetGitHubAccessTokenDemo get GitHub Access Token.
// Parameters:
// resourceGroupName - the resource group name.
// factoryName - the factory name.
// gitHubAccessTokenRequest - get GitHub access token request definition.
func (client FactoriesClient) GetGitHubAccessTokenDemo(ctx context.Context, resourceGroupName string, factoryName string, gitHubAccessTokenRequest GitHubAccessTokenRequest) (result GitHubAccessTokenResponse, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/FactoriesClient.GetGitHubAccessTokenDemo")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: gitHubAccessTokenRequest,
			Constraints: []validation.Constraint{{Target: "gitHubAccessTokenRequest.GitHubAccessCode", Name: validation.Null, Rule: true, Chain: nil},
				{Target: "gitHubAccessTokenRequest.GitHubAccessTokenBaseURL", Name: validation.Null, Rule: true, Chain: nil}}}}); err != nil {
		return result, validation.NewError("datafactory.FactoriesClient", "GetGitHubAccessTokenDemo", err.Error())
	}

	req, err := client.GetGitHubAccessTokenDemoPreparer(ctx, resourceGroupName, factoryName, gitHubAccessTokenRequest)
	if err != nil {
		err = autorest.NewErrorWithError(err, "datafactory.FactoriesClient", "GetGitHubAccessTokenDemo", nil, "Failure preparing request")
		return
	}

	resp, err := client.GetGitHubAccessTokenDemoSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "datafactory.FactoriesClient", "GetGitHubAccessTokenDemo", resp, "Failure sending request")
		return
	}

	result, err = client.GetGitHubAccessTokenDemoResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "datafactory.FactoriesClient", "GetGitHubAccessTokenDemo", resp, "Failure responding to request")
	}

	return
}

// GetGitHubAccessTokenDemoPreparer prepares the GetGitHubAccessTokenDemo request.
func (client FactoriesClient) GetGitHubAccessTokenDemoPreparer(ctx context.Context, resourceGroupName string, factoryName string, gitHubAccessTokenRequest GitHubAccessTokenRequest) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"factoryName":       autorest.Encode("path", factoryName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DataFactory/factories/{factoryName}/getGitHubAccessTokenDemo", pathParameters),
		autorest.WithJSON(gitHubAccessTokenRequest),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// GetGitHubAccessTokenDemoSender sends the GetGitHubAccessTokenDemo request. The method will close the
// http.Response Body if it receives an error.
func (client FactoriesClient) GetGitHubAccessTokenDemoSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// GetGitHubAccessTokenDemoResponder handles the response to the GetGitHubAccessTokenDemo request. The method always
// closes the http.Response Body.
func (client FactoriesClient) GetGitHubAccessTokenDemoResponder(resp *http.Response) (result GitHubAccessTokenResponse, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
