...
// ListXXX lists all of the CDN profiles within an Azure subscription.
func (client ProfilesClient) ListXXX(ctx context.Context) (result ProfileListResultPage, err error) {
    if tracing.IsEnabled() {
        ctx = tracing.StartSpan(ctx, fqdn + "/ProfilesClient.ListXXX")
        defer func() {
            sc := -1
        if result.plr.Response.Response != nil {
        sc = result.plr.Response.Response.StatusCode
        ...
        result.fn = client.listXXXNextResults
    req, err := client.ListXXXPreparer(ctx)
    if err != nil {
    err = autorest.NewErrorWithError(err, "cdn.ProfilesClient", "ListXXX", nil , "Failure preparing request")
    return
    }

        resp, err := client.ListXXXSender(req)
        if err != nil {
        result.plr.Response = autorest.Response{Response: resp}
        err = autorest.NewErrorWithError(err, "cdn.ProfilesClient", "ListXXX", resp, "Failure sending request")
        return
        }

        result.plr, err = client.ListXXXResponder(resp)
        if err != nil {
        err = autorest.NewErrorWithError(err, "cdn.ProfilesClient", "ListXXX", resp, "Failure responding to request")
        }
            if result.plr.hasNextLink() && result.plr.IsEmpty() {
            err = result.NextWithContext(ctx)
            }
    ...
    // ListXXXPreparer prepares the ListXXX request.
    func (client ProfilesClient) ListXXXPreparer(ctx context.Context) (*http.Request, error) {
        pathParameters := map[string]interface{} {
        "subscriptionId": autorest.Encode("path",client.SubscriptionID),
        }
...
autorest.WithQueryParameters(queryParameters))
    ...
    // ListXXXSender sends the ListXXX request. The method will close the
    // http.Response Body if it receives an error.
    func (client ProfilesClient) ListXXXSender(req *http.Request) (*http.Response, error) {
            return client.Send(req, azure.DoRetryWithRegistration(client.Client))
            }

    // ListXXXResponder handles the response to the ListXXX request. The method always
    // closes the http.Response Body.
    func (client ProfilesClient) ListXXXResponder(resp *http.Response) (result ProfileListResult, err error) {
            err = autorest.Respond(
            resp,
            azure.WithErrorUnlessStatusCode(http.StatusOK),
            autorest.ByUnmarshallingJSON(&result),
            ...
            // listXXXNextResults retrieves the next set of results, if any.
            func (client ProfilesClient) listXXXNextResults(ctx context.Context, lastResults ProfileListResult) (result ProfileListResult, err error) {
            req, err := lastResults.profileListResultPreparer(ctx)
            if err != nil {
            return result, autorest.NewErrorWithError(err, "cdn.ProfilesClient", "listXXXNextResults", nil , "Failure preparing next results request")
            }
            if req == nil {
            return
            }
            ...
            if err != nil {
            result.Response = autorest.Response{Response: resp}
            return result, autorest.NewErrorWithError(err, "cdn.ProfilesClient", "listXXXNextResults", resp, "Failure sending next results request")
            }
            result, err = client.ListXXXResponder(resp)
            if err != nil {
            err = autorest.NewErrorWithError(err, "cdn.ProfilesClient", "listXXXNextResults", resp, "Failure responding to next results request")
            }
            return
                    }

            // ListXXXComplete enumerates all values, automatically crossing page boundaries as required.
            func (client ProfilesClient) ListXXXComplete(ctx context.Context) (result ProfileListResultIterator, err error) {
            if tracing.IsEnabled() {
            ctx = tracing.StartSpan(ctx, fqdn + "/ProfilesClient.ListXXX")
            defer func() {
            sc := -1
            ...
            tracing.EndSpan(ctx, sc, err)
                    ...
                    result.page, err = client.ListXXX(ctx)
                            return
            }
