...
// are not currently supported via the API. To add a file to an existing support ticket, visit the [Manage support
...
// consent to share diagnostic information with Azure support is currently not supported via the API. Azure support
...
// [this](https://docs.microsoft.com/azure/azure-resource-manager/management/authenticate-multi-tenant). The primary
// token will be from the tenant for whom a support ticket is being raised against the subscription, i.e. Cloud
...
// Get get ticket details for an Azure subscription. Support ticket data is available for 12 months after ticket
...
// retrieve the next set of support tickets. <br/><br/>Support ticket data is available for 12 months after ticket
// creation. If a ticket was created more than 12 months ago, a request for data might cause an error.
    // Parameters:
        ...
        // with 'eq' operator. For _CreatedDate_ filter, the supported operators are 'gt' and 'ge'. When using both
