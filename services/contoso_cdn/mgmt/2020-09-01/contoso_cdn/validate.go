...
func (client ValidateClient) SecretMethod(ctx context.Context, validateSecretInput ValidateSecretInput) (result ValidateSecretOutput, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("contosocdn.ValidateClient", "SecretMethod", err.Error())
	}

	req, err := client.SecretMethodPreparer(ctx, validateSecretInput)
	if err != nil {
		err = autorest.NewErrorWithError(err, "contosocdn.ValidateClient", "SecretMethod", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "contosocdn.ValidateClient", "SecretMethod", resp, "Failure sending request")
		return
	}

	result, err = client.SecretMethodResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "contosocdn.ValidateClient", "SecretMethod", resp, "Failure responding to request")
		return
	}
