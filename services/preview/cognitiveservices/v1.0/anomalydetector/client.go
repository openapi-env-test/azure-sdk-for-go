...
// ShivangiTestDetectEntireSeries testing This operation generates a model using an entire series, each point is
// detected with the same model. With this method, points before and after a certain point are used to determine
// whether it is an anomaly. The entire detection can give user an overall status of the time series.
// Parameters:
// body - time series points and period if needed. Advanced model parameters can also be set in the request.
func (client BaseClient) ShivangiTestDetectEntireSeries(ctx context.Context, body DetectRequest) (result EntireDetectResponse, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/BaseClient.ShivangiTestDetectEntireSeries")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: body,
			Constraints: []validation.Constraint{{Target: "body.Series", Name: validation.Null, Rule: true, Chain: nil}}}}); err != nil {
		return result, validation.NewError("anomalydetector.BaseClient", "ShivangiTestDetectEntireSeries", err.Error())
	}

	req, err := client.ShivangiTestDetectEntireSeriesPreparer(ctx, body)
	if err != nil {
		err = autorest.NewErrorWithError(err, "anomalydetector.BaseClient", "ShivangiTestDetectEntireSeries", nil, "Failure preparing request")
		return
	}

	resp, err := client.ShivangiTestDetectEntireSeriesSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "anomalydetector.BaseClient", "ShivangiTestDetectEntireSeries", resp, "Failure sending request")
		return
	}

	result, err = client.ShivangiTestDetectEntireSeriesResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "anomalydetector.BaseClient", "ShivangiTestDetectEntireSeries", resp, "Failure responding to request")
	}

	return
}

// ShivangiTestDetectEntireSeriesPreparer prepares the ShivangiTestDetectEntireSeries request.
func (client BaseClient) ShivangiTestDetectEntireSeriesPreparer(ctx context.Context, body DetectRequest) (*http.Request, error) {
	urlParameters := map[string]interface{}{
		"Endpoint": client.Endpoint,
	}
...
// ShivangiTestDetectEntireSeriesSender sends the ShivangiTestDetectEntireSeries request. The method will close the
// http.Response Body if it receives an error.
func (client BaseClient) ShivangiTestDetectEntireSeriesSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}

// ShivangiTestDetectEntireSeriesResponder handles the response to the ShivangiTestDetectEntireSeries request. The method always
// closes the http.Response Body.
func (client BaseClient) ShivangiTestDetectEntireSeriesResponder(resp *http.Response) (result EntireDetectResponse, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
