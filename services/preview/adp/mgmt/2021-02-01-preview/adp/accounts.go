...
// UpdateABDC updates the properties of an existing ADP account
// Parameters:
// resourceGroupName - the name of the resource group. The name is case insensitive.
// accountName - the name of the ADP account
// parameters - the parameters to provide for the current ADP account
func (client AccountsClient) UpdateABDC(ctx context.Context, resourceGroupName string, accountName string, parameters *AccountPatch) (result AccountsUpdateABDCFuture, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/AccountsClient.UpdateABDC")
		defer func() {
			sc := -1
			if result.FutureAPI != nil && result.FutureAPI.Response() != nil {
				sc = result.FutureAPI.Response().StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: accountName,
			Constraints: []validation.Constraint{{Target: "accountName", Name: validation.MaxLength, Rule: 50, Chain: nil},
				{Target: "accountName", Name: validation.Pattern, Rule: `^[a-zA-Z0-9]+(-[a-zA-Z0-9]+)*`, Chain: nil}}}}); err != nil {
		return result, validation.NewError("adp.AccountsClient", "UpdateABDC", err.Error())
	}

	req, err := client.UpdateABDCPreparer(ctx, resourceGroupName, accountName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "adp.AccountsClient", "UpdateABDC", nil, "Failure preparing request")
		return
	}

	result, err = client.UpdateABDCSender(req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "adp.AccountsClient", "UpdateABDC", nil, "Failure sending request")
		return
	}
...
// UpdateABDCPreparer prepares the UpdateABDC request.
func (client AccountsClient) UpdateABDCPreparer(ctx context.Context, resourceGroupName string, accountName string, parameters *AccountPatch) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"accountName":       autorest.Encode("path", accountName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
...
// UpdateABDCSender sends the UpdateABDC request. The method will close the
// http.Response Body if it receives an error.
func (client AccountsClient) UpdateABDCSender(req *http.Request) (future AccountsUpdateABDCFuture, err error) {
	var resp *http.Response
	resp, err = client.Send(req, azure.DoRetryWithRegistration(client.Client))
	if err != nil {
		return
...
// UpdateABDCResponder handles the response to the UpdateABDC request. The method always
// closes the http.Response Body.
func (client AccountsClient) UpdateABDCResponder(resp *http.Response) (result Account, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK, http.StatusCreated),
		autorest.ByUnmarshallingJSON(&result),
