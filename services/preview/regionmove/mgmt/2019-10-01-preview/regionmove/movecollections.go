...
func (client MoveCollectionsClient) BulkRemove(ctx context.Context, resourceGroupName string, moveCollectionName string, body *BulkRemoveRequest) (result MoveCollectionsBulkRemoveFuture, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "regionmove.MoveCollectionsClient", "BulkRemove", nil, "Failure preparing request")
		return
	}

	result, err = client.BulkRemoveSender(req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "regionmove.MoveCollectionsClient", "BulkRemove", result.Response(), "Failure sending request")
		return
	}
