...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/preview/desktopvirtualization/mgmt/2022-02-03-preview/desktopvirtualization"

// AgentUpdatePatchProperties the preferred settings for updating the agent components (RDAgent, Geneva
// Monitoring agent, and side-by-side stack) on session hosts.
type AgentUpdatePatchProperties struct {
	// Type - The preferred mechanism for updating the agent components. This is either Scheduled or Default. Possible values include: 'Default', 'Scheduled'
	Type SessionHostComponentUpdateType `json:"type,omitempty"`
	// UseSessionHostLocalTime - Boolean indicating whether to update the agent components in the local time zone of each session host in the host pool. By default, this is false.
	UseSessionHostLocalTime *bool `json:"useSessionHostLocalTime,omitempty"`
	// MaintenanceWindowTimeZone - The time zone for updating the agent components. Valid time zones can be found here: https://docs.microsoft.com/en-us/dotnet/api/system.timezoneinfo.findsystemtimezonebyid?view=net-5.0. A time zone must be specified if useSessionHostLocalTime is false.
	MaintenanceWindowTimeZone *string `json:"maintenanceWindowTimeZone,omitempty"`
	// MaintenanceWindows - The maintenance windows (day and time) for updating the agent components. At least 1 window must be specified. Optionally, a 2nd window can be specified.
	MaintenanceWindows *[]MaintenanceWindowPatchProperties `json:"maintenanceWindows,omitempty"`
}

// AgentUpdateProperties the preferred settings for updating the agent components (RDAgent, Geneva
// Monitoring agent, and side-by-side stack) on session hosts.
...
type HostPoolPatchProperties struct {
	...
	// PublicNetworkAccess - Enabled to allow this resource to be access from the public network. Possible values include: 'Enabled', 'Disabled', 'EnabledForSessionHostsOnly', 'EnabledForClientsOnly'
	PublicNetworkAccess HostpoolPublicNetworkAccess `json:"publicNetworkAccess,omitempty"`
	// AgentUpdate - The preferred settings for updating the agent components (RDAgent, Geneva Monitoring agent, and side-by-side stack) on session hosts.
...
type HostPoolProperties struct {
	...
	// CloudPcResource - READ-ONLY; Is cloud pc resource.
	CloudPcResource *bool `json:"cloudPcResource,omitempty"`
	// PublicNetworkAccess - Enabled allows this resource to be accessed from both public and private networks, Disabled allows this resource to only be accessed via private endpoints. Possible values include: 'Enabled', 'Disabled', 'EnabledForSessionHostsOnly', 'EnabledForClientsOnly'
	PublicNetworkAccess HostpoolPublicNetworkAccess `json:"publicNetworkAccess,omitempty"`
	// AgentUpdate - The preferred settings for updating the agent components (RDAgent, Geneva Monitoring agent, and side-by-side stack) on session hosts.
	AgentUpdate *AgentUpdateProperties `json:"agentUpdate,omitempty"`
	// PrivateEndpointConnections - READ-ONLY; List of private endpoint connection associated with the specified resource
...
func (hpp HostPoolProperties) MarshalJSON() ([]byte, error) {
	...
	if hpp.AgentUpdate != nil {
		objectMap["agentUpdate"] = hpp.AgentUpdate
	}
	return json.Marshal(objectMap)
}
...
type MaintenanceWindowPatchProperties struct {
	// Hour - The starting hour of the maintenance window (0-23). Note that maintenance windows are 2 hours long. This means that updates can be applied anytime from the specified start hour to 2 hours after.
	Hour *int32 `json:"hour,omitempty"`
	// DayOfWeek - The day of the week (Monday-Sunday). Possible values include: 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'
...
func (shp SessionHostProperties) MarshalJSON() ([]byte, error) {
	...
	if shp.FriendlyName != nil {
		objectMap["friendlyName"] = shp.FriendlyName
	}
	if shp.Status != "" {
