...
// UpdateAutoRenew cancel auto renew for product by product id.
// Parameters:
// billingAccountName - the ID that uniquely identifies a billing account.
// productName - the ID that uniquely identifies a product.
// body - request parameters that are provided to the update auto renew operation.
func (client ProductsClient) UpdateAutoRenew(ctx context.Context, billingAccountName string, productName string, body UpdateAutoRenewRequest) (result UpdateAutoRenewOperation, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ProductsClient.UpdateAutoRenew")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.UpdateAutoRenewPreparer(ctx, billingAccountName, productName, body)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.ProductsClient", "UpdateAutoRenew", nil, "Failure preparing request")
		return
	}

	resp, err := client.UpdateAutoRenewSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "billing.ProductsClient", "UpdateAutoRenew", resp, "Failure sending request")
		return
	}

	result, err = client.UpdateAutoRenewResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.ProductsClient", "UpdateAutoRenew", resp, "Failure responding to request")
	}

	return
}

// UpdateAutoRenewPreparer prepares the UpdateAutoRenew request.
func (client ProductsClient) UpdateAutoRenewPreparer(ctx context.Context, billingAccountName string, productName string, body UpdateAutoRenewRequest) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"billingAccountName": autorest.Encode("path", billingAccountName),
		"productName":        autorest.Encode("path", productName),
	}
...
// UpdateAutoRenewSender sends the UpdateAutoRenew request. The method will close the
// http.Response Body if it receives an error.
func (client ProductsClient) UpdateAutoRenewSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}

// UpdateAutoRenewResponder handles the response to the UpdateAutoRenew request. The method always
// closes the http.Response Body.
func (client ProductsClient) UpdateAutoRenewResponder(resp *http.Response) (result UpdateAutoRenewOperation, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
