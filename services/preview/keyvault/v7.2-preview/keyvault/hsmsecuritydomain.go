...
// certificates - security domain download operation requires customer to provide three certificates containing
// public key in JWK format.
func (client HSMSecurityDomainClient) Download(ctx context.Context, vaultBaseURL string, certificates CertificateSet) (result SecurityDomainObject, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: certificates,
			Constraints: []validation.Constraint{{Target: "certificates.Certificates", Name: validation.Null, Rule: true,
				Chain: []validation.Constraint{{Target: "certificates.Certificates", Name: validation.MaxItems, Rule: 3, Chain: nil},
					{Target: "certificates.Certificates", Name: validation.MinItems, Rule: 3, Chain: nil},
					{Target: "certificates.Certificates", Name: validation.UniqueItems, Rule: true, Chain: nil},
				}}}}}); err != nil {
		return result, validation.NewError("keyvault.HSMSecurityDomainClient", "Download", err.Error())
	}

	req, err := client.DownloadPreparer(ctx, vaultBaseURL, certificates)
	if err != nil {
...
func (client HSMSecurityDomainClient) DownloadPreparer(ctx context.Context, vaultBaseURL string, certificates CertificateSet) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithJSON(certificates),
