...
func (client *TableResourcesClient) BeginRetrieveContinuousBackupInformation(ctx context.Context, resourceGroupName string, accountName string, tableName string, location ContinuousBackupRestoreLocation, options *TableResourcesClientBeginRetrieveContinuousBackupInformationOptions) (TableResourcesClientRetrieveContinuousBackupInformationPollerResponse, error) {
	resp, err := client.retrieveContinuousBackupInformation(ctx, resourceGroupName, accountName, tableName, location, options)
	if err != nil {
		return TableResourcesClientRetrieveContinuousBackupInformationPollerResponse{}, err
	}
	result := TableResourcesClientRetrieveContinuousBackupInformationPollerResponse{
		RawResponse: resp,
	}
	pt, err := armruntime.NewPoller("TableResourcesClient.RetrieveContinuousBackupInformation", "location", resp, client.pl)
	if err != nil {
		return TableResourcesClientRetrieveContinuousBackupInformationPollerResponse{}, err
	}
	result.Poller = &TableResourcesClientRetrieveContinuousBackupInformationPoller{
		pt: pt,
	}
	return result, nil
}
