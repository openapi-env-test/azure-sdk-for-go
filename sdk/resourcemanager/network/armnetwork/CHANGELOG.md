...
- Const `ApplicationGatewaySKUNameBasic` from type alias `ApplicationGatewaySKUName` has been removed
- Const `ApplicationGatewayTierBasic` from type alias `ApplicationGatewayTier` has been removed
- Const `NetworkInterfaceAuxiliaryModeAcceleratedConnections` from type alias `NetworkInterfaceAuxiliaryMode` has been removed
- Const `WebApplicationFirewallRuleTypeRateLimitRule` from type alias `WebApplicationFirewallRuleType` has been removed
- Type alias `AdminState` has been removed
- Type alias `ApplicationGatewayFirewallRateLimitDuration` has been removed
- Type alias `ApplicationGatewayFirewallUserSessionVariable` has been removed
- Type alias `AzureFirewallPacketCaptureFlagsType` has been removed
- Type alias `NetworkInterfaceAuxiliarySKU` has been removed
- Type alias `PublicIPAddressDNSSettingsDomainNameLabelScope` has been removed
- Type alias `ScrubbingRuleEntryMatchOperator` has been removed
- Type alias `ScrubbingRuleEntryMatchVariable` has been removed
- Type alias `ScrubbingRuleEntryState` has been removed
- Type alias `SyncMode` has been removed
- Type alias `WebApplicationFirewallScrubbingState` has been removed
...
- Struct `AzureFirewallsClientPacketCaptureResponse` has been removed
...
- Struct `VirtualApplianceConnectionsClientCreateOrUpdateResponse` has been removed
- Struct `VirtualApplianceConnectionsClientDeleteResponse` has been removed
- Struct `VirtualApplianceConnectionsClientListResponse` has been removed
...
- Field `EnableKerberos` of struct `BastionHostPropertiesFormat` has been removed
- Field `NetworkACLs` of struct `BastionHostPropertiesFormat` has been removed
- Field `VirtualNetwork` of struct `BastionHostPropertiesFormat` has been removed
...
- Field `FileUploadEnforcement` of struct `PolicySettings` has been removed
- Field `LogScrubbing` of struct `PolicySettings` has been removed
- Field `RequestBodyEnforcement` of struct `PolicySettings` has been removed
- Field `RequestBodyInspectLimitInKB` of struct `PolicySettings` has been removed
...
- Field `AdditionalNics` of struct `VirtualAppliancePropertiesFormat` has been removed
- Field `VirtualApplianceConnections` of struct `VirtualAppliancePropertiesFormat` has been removed
...
- Field `AdminState` of struct `VirtualNetworkGatewayPropertiesFormat` has been removed
- Field `AutoScaleConfiguration` of struct `VirtualNetworkGatewayPropertiesFormat` has been removed
- Field `GroupByUserSession` of struct `WebApplicationFirewallCustomRule` has been removed
- Field `RateLimitDuration` of struct `WebApplicationFirewallCustomRule` has been removed
- Field `RateLimitThreshold` of struct `WebApplicationFirewallCustomRule` has been removed

### Features Added

- New value `ApplicationGatewayCustomErrorStatusCodeHTTPStatus499` added to type alias `ApplicationGatewayCustomErrorStatusCode`
- New value `LoadBalancerBackendAddressAdminStateDrain` added to type alias `LoadBalancerBackendAddressAdminState`
...
- New field `LocalAddressSpace` in struct `VirtualNetworkPeeringPropertiesFormat`
- New field `LocalSubnetNames` in struct `VirtualNetworkPeeringPropertiesFormat`
- New field `LocalVirtualNetworkAddressSpace` in struct `VirtualNetworkPeeringPropertiesFormat`
- New field `PeerCompleteVnets` in struct `VirtualNetworkPeeringPropertiesFormat`
- New field `RemoteSubnetNames` in struct `VirtualNetworkPeeringPropertiesFormat`
